...
const GPT_API_ENDPOINT = process.env.VUE_APP_GPT_API_ENDPOINT

class GptManager {
  sampleTexts = [
    {
      'id': 1, 'title': 'Meeting: 90% Accuracy',
      'text':
`Jason: So what do you think you add to this Next? What are the features you add next to weight room 
...
Vinny: Yeah. It's great.`
      ...
      'id': 2, 'title': 'Meeting: Agora 95% Accuracy',
      'text': 
`Jason: So what do you think you add to this next, What are the features you add next, The weight room.   
...
Jason：Yeah, that's great.`
      ...
      'id': 3, 'title': 'Language Learning: 90% Accuracy',
      'text':
`Mantho: Hello? 
...
Mantho: Okay. Alright.`
      ...
      'id': 4, 'title': 'Language Learning: Agora 95% Accuracy',
      'text':
`Mantho: Hello.
...
Mantho: Okay. All right. All right.`
  ...
  async runAnalysis(prompt, text, callback) {
    // console.log(prompt)
    // console.log(text)
    ...
    axios.post(GPT_API_ENDPOINT, data, {headers: headers}).then((res) => {
      console.log(res.data)
      if (res.status == '200') {
        this.fullText = res.data.result
        callback(true, this.fullText)
      } else {
        callback(false, null)
      }
    }).catch(error => {
...
// export Gpt Manager 
let gptMgr = new GptManager()
export default gptMgr
